...
writeRegister(_address, AVERAGE_REG, 0x09); // fast, low power
...
byte HTS221::readRegister(byte slaveAddress, byte regToRead)
{
    ...
    Wire.requestFrom((uint8_t)slaveAddress, (uint8_t)1); //Ask for 1 byte, once done, bus is released by default

   for (int i = 0; ; i++) {
      if (Wire.available()) break; //Wait for the data to come back
      if (i == 100000)
        return 0xff;
   }
